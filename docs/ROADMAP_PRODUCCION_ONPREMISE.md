...
| 2026-09-21 | Commit `69f65df5abb1bf1b5adb5308ee82bd3f34c05502` publicado y sincronizado; extracción real comparativa recuperó períodos 2020/2019, USD, cero filas sospechosas y cuadratura final con diferencia 0; seguridad, dependencias, CodeQL y smoke Docker con backup/restore aprobaron en CI; corrección local del preflight separa exclusiones, clasificables, códigos desconocidos y conflictos sobre una sola instantánea | Publicar la corrección del preflight previa aprobación; repetir el gate hasta obtener estado verde; verificar el despliegue exacto en Render y ejecutar la validación humana del piloto | Release gate `35450210598` falló en `Verify Neon and pipeline knowledge`: Neon reportó 1.205 entradas y el pipeline 1.203; el despliegue, la recertificación firmada y el registro del commit fueron omitidos; la corrección local aún no ha sido validada contra Neon | Local después de la corrección: 1.818 recolectadas, 1.799 aprobadas, 19 omitidas y 3 advertencias; 50 pruebas dirigidas de Neon, release, atestación y Render aprobadas; `ruff` y `git diff --check` aprobados | Revisar la selección local, publicar sólo con aprobación, observar el nuevo diagnóstico de Neon en CI y continuar únicamente si el release gate completo queda verde |
