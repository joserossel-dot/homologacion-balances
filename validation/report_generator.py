...
class ReportGenerator:
    ...
    def generate_per_balance_report(result: ValidationResult) -> str:
        ...
        lines.append(f"  Detail accounts:   {result.accounts_total_detail}")
        lines.append(f"  Specific:          {result.accounts_classified_specific}")
        lines.append(f"  Residual:          {result.accounts_classified_residual}")
        lines.append(f"  Unclassified:      {result.accounts_unclassified}")
        lines.append(f"  Pending review:    {result.accounts_pending_review}")
        lines.append(f"  Controls:          {result.accounts_controls}")
    ...
    def generate_global_report(all_results: list[ValidationResult], output_path: str | Path):
        ...
        if all_results:
            ...
            lines.append(f"- Detail accounts: {worst.accounts_total_detail}")
            lines.append(f"- Specific: {worst.accounts_classified_specific}")
            lines.append(f"- Residual: {worst.accounts_classified_residual}")
            lines.append(f"- Unclassified: {worst.accounts_unclassified}")
