...
def _print_summary(metrics: dict[str, Any], report_path: Path) -> None:
    ...
    print(f"  Cuentas detalle: {metrics['accounts_total_detail']}")
    print(f"  Específicas:     {metrics['accounts_classified_specific']}")
    print(f"  Residuales:      {metrics['accounts_classified_residual']}")
    print(f"  Sin clasificar:  {metrics['accounts_unclassified']}")
    print(f"  Pendientes:      {metrics['accounts_pending_review']}")
    print(f"  Controles:       {metrics['accounts_controls']}")
